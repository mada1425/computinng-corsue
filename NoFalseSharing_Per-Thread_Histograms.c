...
#define PAD_ELEMS (CACHE_LINE_SIZE / sizeof(int))
#define ROW (NUM_RESOURCES + PAD_ELEMS)
...
int main(int argc, char **argv) {
    ...
    if (n == 0) { printf("[padded] empty or format mismatch\n"); fclose(f); return 0; }
 ...
 idx[i] = k;    }
    ...
    int *counts = (int*)calloc((size_t)T * ROW, sizeof(int));
    ...
    printf("[Per-Thread PADDED ] threads=%d  time=%.8f s\n", T, t1 - t0);
    for (int k = 0; k < NUM_RESOURCES; k++)
        printf("%s : %d\n", RES[k], total[k]);
